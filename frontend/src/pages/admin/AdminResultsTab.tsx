...
import { RefreshCw, FolderTree, Target, Hourglass, Pencil, Ban, Coins, Trash2 } from "lucide-react";
...
export function AdminResultsTab() {
  ...
  const handleRecalculate = async () => {
    ...
    try {
      ...
      setRecalcError(e instanceof ApiError ? e.message : "Recalculate failed — check the API server log.");
    } finally {
  ...
  const handleRebuild = async () => {
    ...
    try {
      ...
      setRebuildMessage(`Done — ${r.written} record(s) written.`);
      reload();
    } catch (e) {
      setRebuildError(e instanceof ApiError ? e.message : "Rebuild failed — check the API server log.");
    } finally {
      setRebuilding(false);
    }
  };
    ...
    <div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="flex items-center gap-1.5 font-semibold">
              <RefreshCw size={15} /> Recalculate All Points
            </p>
      ...
      <div className="mb-6 rounded-lg border border-gray-200 p-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="flex items-center gap-1.5 font-semibold">
              <FolderTree size={15} /> Rebuild match_players
            </p>
            <p className="text-sm text-gray-500">
          ...
          <button
            ...
            disabled={rebuilding}
            className="btn-raised rounded border border-gray-300 px-4 py-2 text-sm font-semibold hover:bg-gray-100 disabled:opacity-50"
          >
      ...
      <h2 className="mb-2 flex items-center gap-2 text-lg font-bold">
        <Target size={18} /> Awaiting Result Entry
      </h2>
      ...
      <h2 className="mb-2 flex items-center gap-2 text-lg font-bold">
        <Hourglass size={18} /> Voting Still Open
      </h2>
      ...
      <h2 className="mb-2 flex items-center gap-2 text-lg font-bold">
        <Pencil size={18} /> Update / Correct Result
      </h2>
    ...
    <div className="rounded-lg border border-gray-200 p-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="btn-raised rounded bg-[#28324f] px-3 py-1.5 text-sm font-semibold text-white hover:bg-[#1c2439] disabled:opacity-50"
        >
          Save Result
        </button>
      </div>
...
function PenaltiesSection({
  ...
  return (
    <div>
      <h2 className="mb-2 flex items-center gap-2 text-lg font-bold">
        <Coins size={18} /> Manual Penalties
      </h2>
      ...
      <div className="mb-4 flex flex-wrap items-end gap-2 rounded-lg border border-gray-200 p-4">
        ...
        <button
          onClick={handleAdd}
          className="btn-raised rounded bg-[#28324f] px-3 py-2 text-sm font-semibold text-white hover:bg-[#1c2439]"
        >
          Add Penalty
        </button>
      </div>
        ...
        <div className="max-h-80 space-y-1 overflow-y-auto rounded-md border border-gray-200 p-2">
          {penalties.map((p) => (
            ...
            >
              ...
              <span className="font-bold text-rose-700">-{p.points.toFixed(2)}</span>
